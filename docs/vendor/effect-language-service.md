		...
		"plugins": [
			{
				...
				"diagnosticSeverity": {
					// allows to change per-rule default severity of the diagnostic in the whole project
...
```ts
export class UserRepo extends Repository("Hello")<
	UserRepo,
	{
		/** ... */
	}
...
{
	"compilerOptions": {
		"plugins": [{ "name": "typescript-svelte-plugin" }, { "name": "@effect/language-service" }]
	}
}
```
