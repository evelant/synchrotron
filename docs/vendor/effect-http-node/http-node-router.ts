...
const HttpLive = HttpRouter.empty.pipe(
	HttpRouter.get(
		"/",
		Effect.map(HttpServerRequest.HttpServerRequest, (req) => HttpServerResponse.text(req.url))
	),
	HttpRouter.get("/healthz", HttpServerResponse.text("ok").pipe(HttpMiddleware.withLoggerDisabled)),
	HttpRouter.post(
		"/upload",
		Effect.gen(function* () {
