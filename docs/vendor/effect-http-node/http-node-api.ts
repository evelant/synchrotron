...
class UsersApi extends HttpApiGroup.make("users")
	.add(
		HttpApiEndpoint.get("findById")`/${HttpApiSchema.param("id", Schema.NumberFromString)}`
			...
			.addError(Schema.String.pipe(HttpApiSchema.asEmpty({ status: 413, decode: () => "boom" })))
	)
	.add(
...
class TopLevelApi extends HttpApiGroup.make("topLevel", { topLevel: true })
	...
	.add(HttpApiEndpoint.get("binary", "/binary").addSuccess(HttpApiSchema.Uint8Array()))
	.add(
		HttpApiEndpoint.get("urlParams", "/url-params").addSuccess(
			Schema.Struct({
...
class MyApi extends HttpApi.make("api").add(UsersApi).add(TopLevelApi).addHttpApi(AnotherApi) {}
...
const PeopleLive = HttpApiBuilder.group(MyApi, "people", (handlers) =>
	handlers.handle("list", (_) => Effect.succeed([new User({ id: 1, name: "John" })]))
)

const TopLevelLive = HttpApiBuilder.group(MyApi, "topLevel", (handlers) =>
	handlers
...
const ApiLive = HttpApiBuilder.api(MyApi).pipe(Layer.provide([UsersLive, TopLevelLive, PeopleLive]))
