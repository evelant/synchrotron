...
interface TodoService {
	readonly create: (
		...
		Todo,
		HttpClientError.HttpClientError | HttpBody.HttpBodyError | ParseResult.ParseError
	>
}
...
Effect.flatMap(TodoService, (todos) =>
	todos.create({ userId: 1, title: "test", completed: false })
).pipe(Effect.tap(Effect.log), Effect.provide(TodoServiceLive), runMain)
