...
- Local OpenTelemetry backend (Docker): Grafana LGTM stack on `http://localhost:3001` (override with `OTEL_LGTM_GRAFANA_PORT`)
...
- Ensure `pnpm docker:up` is running (includes `grafana/otel-lgtm`).
