export default `CREATE OR REPLACE FUNCTION rollback_to_action(p_action_id TEXT) -- p_action_id can be NULL
RETURNS VOID AS $$
DECLARE
...
BEGIN
    ...
    IF p_action_id IS NOT NULL THEN
	    ...
	    -- 2. Find AMRs to reverse and Action IDs to unapply.
	    -- Only roll back actions that are currently marked as applied in local_applied_action_ids.
	    WITH applied_actions_to_rollback AS (
	    ...
	    SELECT
	        ...
	        array_agg(atr.action_id)
	    INTO
	        amr_ids_to_reverse,
	        action_ids_to_unapply
	    FROM action_modified_rows amr
	    JOIN applied_actions_to_rollback atr ON amr.action_record_id = atr.action_id;
