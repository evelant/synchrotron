...
- uploads include `basisServerIngestId` (client's last seen `server_ingest_id` cursor); the server rejects uploads when the client is behind the server ingestion head (fetch+reconcile+retry; the sync loop (`performSync()` / `requestSync()`) does a small bounded retry; RPC error `_tag` = `SendLocalActionsBehindHead`)
...
- Sync engine (`SyncService`): implemented as internal modules under `packages/sync-core/src/sync/` and wired in `packages/sync-core/src/SyncService.ts`. Transports/examples should trigger sync via `SyncService.requestSync()` (single-flight + burst coalescing) rather than calling `performSync()` directly.
...
- Sync transport (`SyncNetworkService`): sync-core defines the RPC transport contract for uploads + pull-based remote ingress (no built-in live implementation); client runtimes provide concrete layers (HTTP RPC, tests). `sync-core` owns ingestion of fetched rows into `action_records` / `action_modified_rows` so transports don’t need DB-specific upsert logic. Consumer apps can provide a custom transport by passing `transport` (type `SynchrotronTransport`) to `makeSynchrotronClientLayer(...)`. Push/notify/hybrid transports provide `SyncIngress` (a stream of action-log batches / wakeups) via `transport.syncIngressLayer`; when present, `makeSynchrotronClientLayer(...)` automatically enables the core-owned ingress runner. For a reference push-ingress transport, see `ElectricTransport` in `@synchrotron/sync-client/transports`. In Electric-enabled clients, Electric provides `SyncIngress` and `fetchRemoteActions()` is metadata-only (epoch/retention) via `FetchRemoteActions(mode="metaOnly")` to avoid “two ingress writers”.
- Trigger system:
...
- Sync phases are instrumented with `Effect.withSpan(...)` and log annotations (for example: `clientId`, `syncSessionId`, `applyBatchId`, `sendBatchId`) so logs can be correlated across fetch/apply/send/reconcile. In development, these spans can be exported via OpenTelemetry (see `@synchrotron/observability` + the example backend’s local Grafana/LGTM stack).
- Example apps can also export Effect logs to Loki via OTLP (enable with `*_OTEL_LOGS_ENABLED=true`).
- Sync-core records low-cardinality Effect metrics (counters/timers/gauges) for sync + RPC health. Example apps can export these metrics via OTLP (enable with `*_OTEL_METRICS_ENABLED=true`).
...
- Transports/apps “kick” the runtime via `SyncService.requestSync()` (single-flight + burst coalescing). For push/notify ingresses, transports emit `SyncIngress` events; when a client is constructed with a `transport` that provides `syncIngressLayer`, the sync-client layer automatically enables the core runner (ingestion + `requestSync()` triggers).
