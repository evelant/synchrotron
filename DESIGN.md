...
- Sync transport (`SyncNetworkService`): uploads local unsynced actions/AMRs and (optionally) performs RPC-based remote ingress. In Electric-enabled clients, remote ingress is owned by `ElectricSyncService` and `fetchRemoteActions()` is metadata-only (epoch/retention) to avoid “two ingress writers”.
- Trigger system:
