...
- Sync phases are instrumented with `Effect.withSpan(...)` and log annotations (for example: `clientId`, `syncSessionId`, `applyBatchId`, `sendBatchId`) so logs can be correlated across fetch/apply/send/reconcile. In development, these spans can be exported via OpenTelemetry (see `@synchrotron/observability` + the example backend’s local backends: Grafana/LGTM or OpenObserve).
